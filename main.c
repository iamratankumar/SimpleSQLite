...
void*get_page(Pager*pager, uint32_t page_num) {
    if (page_num>TABLE_MAX_PAGES) {
        printf("Tried to fetch page page number out of bound. %d > %d\n",page_num, TABLE_MAX_PAGES);
        exit(EXIT_FAILURE);
    }
...
void db_close(Table* table) {
    ...
    uint32_t n_additional_rows = table->num_rows %ROWS_PER_PAGE;
    if (n_additional_rows>0) {
        uint32_t page_num = num_full_pages;
        if (pager->pages[page_num] != NULL) {
            pager_flush(pager, page_num, n_additional_rows* ROW_SIZE);
...
void curser_advance(Cursor* cursor) {
    cursor->row_num +=1;
...
void print_prompt() { printf("SimpleSQL > "); }
...
PrepareResult prepare_insert (InputBuffer* input_buffer, Statement* statement) {
    ...
    if (
        id_string == NULL||
        username == NULL ||
        email == NULL) return PREPARE_SYNTAX_ERROR;
    ...
    statement->row_toi.id = id;
    strcpy(statement->row_toi.username, username);
    strcpy(statement->row_toi.email, email);
...
ExecuteResults execute_insert(Statement* statement, Table* table) {
    ...
    Row* row_to_insert = &(statement->row_toi);
...
ExecuteResults execute_select(Statement* statement, Table* table) {
